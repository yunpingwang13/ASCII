...
public class CharFitter {
	...
	public int similarity(int source, int comp){
		int rdiff = Math.abs((source & 0x00ff0000) >> 16 - (comp & 0x00ff0000) >> 16);
		int gdiff = Math.abs((source & 0x0000ff00) >> 8 - (comp & 0x0000ff00) >> 8);
...
//				for(int p = 0;p<chHeight;p++){
//					for(int q = 0;q<chWidth;q++){
//						System.out.format("%x ",arr.data[p+chHeight*i][q+chWidth*j]);
//					}
//					System.out.println();
//				}
				result[i][j] = maxChar;
